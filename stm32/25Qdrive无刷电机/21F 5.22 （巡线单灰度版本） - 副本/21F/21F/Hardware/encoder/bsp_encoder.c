...
#define WHEEL_DIAMETER_MM 95.5f
#define PI_F 3.1415926f
#define WHEEL_CIRCUMFERENCE_MM 300   // 3.14 * 95.5 == 300mm
#define MM_PER_ENCODER_COUNT ((float)WHEEL_CIRCUMFERENCE_MM / COUNT_PER_REV)
...
void Encoder_Update(void)
{
    ...
    encoder_left.speed_mm_s = Encoder_CountToSpeedMmS(encoder_left.diff_cnt);
    encoder_right.speed_mm_s = Encoder_CountToSpeedMmS(encoder_right.diff_cnt);
...
{
    return (float)cnt * MM_PER_ENCODER_COUNT;
}

float Encoder_CountToSpeedMmS(int16_t cnt)
{
    return (float)cnt * MM_PER_ENCODER_COUNT / SAMPLE_PERIOD_S;
}
