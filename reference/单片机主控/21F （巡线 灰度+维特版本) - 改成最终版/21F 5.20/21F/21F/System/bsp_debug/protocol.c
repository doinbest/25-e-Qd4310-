...
int8_t receiving_process(void)
{
 ...
 while(1)
 {
   ...
   switch (cmd_type)
   {
     ...
     case START_CMD:    // 等待修改 TB6612 如何启动？直接给速度似乎就可以了
     {
        //  car_go = 1;
        //  if (Car_Mode == Speed_Mode)
        //  {
        //      pid_speed.integral = 0; // 启动前清零积分项
        //  }
        // else if(Car_Mode == Speed2_Mode)
        // {
        //     pid_speed2.integral = 0;    // 启动前清零积分项
        // }
        // else if(Car_Mode == Location_Mode)
        // {
        //     pid_location.integral = 0;    // 启动前清零积分项
        // }       
     }
     break;
